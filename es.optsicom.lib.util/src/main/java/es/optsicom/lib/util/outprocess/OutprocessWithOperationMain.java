...
public class OutprocessWithOperationMain {
	...
	private static final Logger logger = Logger.getLogger(OutprocessWithOperationMain.class.getName());
	...
	public static void main(String[] args) throws Exception {
		...
		try (ServerSocket serverSocket = new ServerSocket()) {
			...
			ObjectOutputStream sendToStream = new ObjectOutputStream(socket.getOutputStream());

			ObjectInputStream receiveFromStream = new ObjectInputStream(socket.getInputStream());

			Object remoteObject = receiveFromStream.readObject();

			while (true) {

				OperationPacket operationPacket = (OperationPacket) receiveFromStream.readObject();

				if (operationPacket != null) {
					Object result = execOperation(remoteObject, operationPacket.getOperationName(),
	...
	private static Object execOperation(Object remoteObject, String operationName, Object[] params)
		...
		if (remoteObject instanceof OperationProcessor) {
			...
			for (int i = 0; i < params.length; i++) {
				paramClasses[i] = (params[i] != null) ? params[i].getClass() : null;
			}
			...
			if (method == null) {
				throw new NoSuchMethodException(
						"Method " + operationName + " not fount in class " + remoteObject.getClass().getName());
			}
