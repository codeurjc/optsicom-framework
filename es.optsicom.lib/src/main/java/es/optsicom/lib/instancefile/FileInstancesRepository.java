...
public abstract class FileInstancesRepository extends InstancesRepository {
	...
	protected void populate() {
		...
		for (File fileSetDirectory : directoriesWithFiles) {
			...
			String instanceSetId = fileSetDirectory.getAbsolutePath().substring(instanceFilesDir.getAbsolutePath().length()+1);
			instanceFileSet.setId(instanceSetId.replace(File.separatorChar, InstancesRepository.INSTANCE_ID_PATH_SEPARATOR.charAt(0)));
