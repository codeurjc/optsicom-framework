...
public class NMovImprovementMethod<S extends Solution<I>, I extends Instance> extends MovementImprovementMethod<S, I>
	...
	private void applyMovement() {
		...
		if (!MathUtil.efectiveEquals(originalWeight + bestIncrement, solution.getWeight())) {
			throw new RuntimeException("Applying the movement " + ArraysUtil.toStringObj(movementAttributes)
					+ " doesn't increment the solution value as expected. It should be "
					+ (originalWeight + bestIncrement) + " and is " + solution.getWeight());
		}
	...
	public void testMovement(double increment, Object movementAttributes) {
		...
		if (this.movementAttributes == null || bestMode.isNoWorseThanPerc(this.originalWeight + increment,
				this.originalWeight + bestIncrement, beta)) {
	...
	public static <S extends Solution<I>, I extends Instance> NMovImprovementMethod<S, I> create(
			MovementGenerator<S, I> movementGenerator, Mode mode, int maxNumMovements) {
