...
public class Execution {
	...
	@JsonCreator
	public Execution(@JsonProperty("experiment") Experiment experiment,
			@JsonProperty("instance") InstanceDescription instance, @JsonProperty("method") MethodDescription method,
			@JsonProperty("timeLimit") long timeLimit, @JsonProperty("numExecution") int numExecution) {
		super();
	...
	public Event getLastEvent(String eventName, long timelimit) {
		...
		for (int j = events.size() - 1; j >= 0; j--) {
			...
			if (anEvent.getTimestamp() <= timelimit
					&& (event == null || anEvent.getTimestamp() > event.getTimestamp())) {
