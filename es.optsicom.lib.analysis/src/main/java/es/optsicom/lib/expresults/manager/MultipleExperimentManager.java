...
/**
 * Creates a new ExperimentManager as fusion of several ExperimentManagers. This class is useful when
 * an experiment is run in different machines and ends up stored as multiple entries in the experiment
 * table. This class considers all the experiments specified as if it was a single experiment. In this sense, 
 * it is considered that different experiments can contain the same method and instances are sum up.
 *
...
public class MultipleExperimentManager implements ExperimentManager {
	...
	public Experiment getExperiment() {
		...
		Experiment experiment = new Experiment(this.getName(), firstExperiment.getResearcher(), new Date(0), firstExperiment.getComputer());
