...
public class JSONExperimentLoader {
	...
	public static MemoryExperimentManager load(File jsonFile) throws IOException, ParseException {
		return new JSONExperimentLoader().internalLoad(jsonFile);
	}

	private MemoryExperimentManager internalLoad(File jsonFile)
		...
		while (it.hasNext()) {
			...
			InstanceDescription id = new InstanceDescription(new DBProperties(properties));
	...
	private void loadExperiment(JsonNode exp) throws ParseException {
		...
		ComputerDescription computerDescription = new ComputerDescription(
				exp.path("computer").path("name").getValueAsText());
	...
	private Map<String, String> getNodeProperties(JsonNode instance) {
		Iterator<Entry<String, JsonNode>> propIt = instance.path("properties").getFields();
	...
	private Map<MethodDescription, List<Execution>> loadExecutions(JsonNode instanceNode,
	...
	private List<Execution> loadExecutions(JsonNode methodNode, MethodDescription method,
			InstanceDescription instance) {
	...
	private void loadEvents(Execution execution, JsonNode executionNode) {
		...
		while (it.hasNext()) {
			...
			if (eventNameNode.getValueAsText().equals("objValue")) {
				DoubleEvent e = new DoubleEvent(execution, eventNode.path("timestamp").getValueAsLong(),
						eventNode.path("eventName").getValueAsText(), eventNode.path("value").getValueAsDouble());
				execution.addEvent(e);
			} else if (eventNameNode.getValueAsText().equals("finishTime")) {
				NonValueEvent e = new NonValueEvent(execution, eventNode.path("timestamp").getValueAsLong(),
						eventNode.path("eventName").getValueAsText());
				execution.addEvent(e);
			} else {
				throw new Error("Unexpected event type: " + eventNameNode.getValueAsText());
			}
		}
