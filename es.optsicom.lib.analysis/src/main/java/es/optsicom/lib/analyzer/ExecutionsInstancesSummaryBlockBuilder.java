...
public class ExecutionsInstancesSummaryBlockBuilder extends BlockBuilder {
	...
	private Table createAllInstancesTable(ExperimentManager experimentResults) {
		StatisticGroup sg1 = StatisticGroup
		.createMultipleStatisticGroup(new LastEventRP(Event.OBJ_VALUE_EVENT, SummarizeMode.MAX, -1, Source.VALUE),
				new Statistic[] {
						new Statistic(new DevStatisticCalc(experimentResults.getProblemBestMode()),
								"Dev (Max)"),
						new Statistic(
								new NumBestStatisticCalc(experimentResults.getProblemBestMode()),
								"#Best (Max)"),
						new Statistic(new ScoreStatisticCalc(experimentResults.getProblemBestMode()),
								"Score (Max)"), });
		StatisticGroup sg2 = StatisticGroup
		.createMultipleStatisticGroup(new LastEventRP(Event.OBJ_VALUE_EVENT, SummarizeMode.MIN, -1, Source.VALUE),
				new Statistic[] {
						new Statistic(new DevStatisticCalc(experimentResults.getProblemBestMode()),
								"Dev (Min)"),
						new Statistic(
		...
		StatisticGroup sg3 = StatisticGroup
		.createMultipleStatisticGroup(new LastEventRP(Event.OBJ_VALUE_EVENT, SummarizeMode.AVERAGE, -1, Source.VALUE),
				new Statistic[] {
						new Statistic(new DevStatisticCalc(experimentResults.getProblemBestMode()),
								"Dev (Avg)"),
						new Statistic(
		...
		StatisticGroup sg4 = StatisticGroup.createMultipleStatisticGroup(new LastEventRP(
