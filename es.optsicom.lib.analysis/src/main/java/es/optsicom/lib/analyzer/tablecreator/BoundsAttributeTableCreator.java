...
public class BoundsAttributeTableCreator extends AttributedTableCreator {
	...
	public AttributedTable buildTable() {
		...
		this.addStatisticGroup(StatisticGroup.createMultipleStatisticGroup(new GapRP(),
				new Statistic[] { new Statistic(
						new NonRelativizerStatisticCalc(SummarizeMode.AVERAGE, NumberType.PERCENT), "Gap") },
				new Statistic[] { new Statistic(new NonRelativizerStatisticCalc(SummarizeMode.SUM, NumberType.INTEGER),
						"#Opt") }));

		this.addStatisticGroup(
				StatisticGroup.createMultipleStatisticGroup(new LastEventRP("lowerBound"),
						new Statistic[] { new Statistic(
								new NonRelativizerStatisticCalc(SummarizeMode.AVERAGE, NumberType.DECIMAL),
								"Lower Bound") }));

		this.addStatisticGroup(
				StatisticGroup.createMultipleStatisticGroup(new LastEventRP("upperBound"),
						new Statistic[] { new Statistic(
								new NonRelativizerStatisticCalc(SummarizeMode.AVERAGE, NumberType.DECIMAL),
								"Upper Bound") }));

		this.addStatisticGroup(
				StatisticGroup.createMultipleStatisticGroup(new LastEventRP(Event.OBJ_VALUE_EVENT),
						new Statistic[] { new Statistic(
								new NonRelativizerStatisticCalc(SummarizeMode.AVERAGE, NumberType.DECIMAL),
								"SolutionValue") }));
