...
public class InstancesPropertiesBlockBuilder extends BlockBuilder {
	...
	public InstancesPropertiesBlockBuilder() {
		propsToFilterOut.addAll(Arrays.asList("usecase", "name", "class", "filename", "num"));
	}
	...
	public void buildPages(ExperimentManager experimentResults) {
		...
		List<String> propNames = new ArrayList<String>(instance.getProperties().keySet());
		...
		List<List<String>> allPropertyCombinations = CombinationGenerator.createCombinations(propNames);
		...
		for (List<String> propCombination : allPropertyCombinations) {
			...
			Table table = createPropertiesTable(experimentResults, propCombination);
			reportPage.addReportElement(table);

			// List<Table> tempEvolutionTables =
			// createTempEvolutionTables(experimentResults, propCombination);
	...
	private List<Table> createTempEvolutionTables(ExperimentManager experimentResults, List<String> properties) {
		...
		tableCreator.setInstanceGroupMaker(InstanceGroupMaker.getGroupBy(properties.toArray(new String[0])));
		...
		TempEvolutionApproxAttributeTableCreator feasTableCreator = new TempEvolutionApproxAttributeTableCreator(
				timeLimit, feasStatisticCalc);

		feasTableCreator.setInstanceGroupMaker(InstanceGroupMaker.getGroupBy(properties.toArray(new String[0])));
	...
	private Table createPropertiesTable(ExperimentManager experimentResults, List<String> properties) {
	...
	private void filterProperties(List<InstanceDescription> instances, List<String> propNames) {
		...
		for (Iterator<String> it = propNames.iterator(); it.hasNext();) {
			...
			for (int i = 1; i < instances.size(); i++) {
				if (instances.get(i).getProperties().get(propName) == null
						|| !instances.get(i).getProperties().get(propName).equals(value)) {
					sameValue = false;
					break;
				}
			}
