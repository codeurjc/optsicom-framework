...
public class TempEvolutionApproxAttributeTableCreator extends
		AttributedTableCreator {
	...
	/**
	 * Creates a new object for building time tables with the specified time limit.
	 * @param executionTime Execution time limit in milliseconds to consider for table.
	 */
	...
	public AttributedTable buildTable() {
		...
		if(statistic == null) {
			...
			((DevStatisticCalc)statistic)
					.setRelativeValueProvider(new LastRPRelativeValueProvider(
							new LastEventRP(Event.OBJ_VALUE_EVENT),
							0,
							bestMode == BestMode.MAX_IS_BEST ? SummarizeMode.MAX
									: SummarizeMode.MIN));			
		}
		...
		for (int i = 0; i < numSteps; i++) {
			...
			statisticGroups.add(StatisticGroup.createMultipleStatisticGroup(
					new LastEventRP(Event.OBJ_VALUE_EVENT, timelimit),
					new Statistic[] { new Statistic(statistic, Long
							.toString(timelimit)) }));
		}
